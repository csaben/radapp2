...
# os.system("export QT_QPA_PLATFORM_PLUGIN=offscreen")
...
# tf.config.experimental.set_visible_devices(['/device:GPU:0'], 'GPU')
...
input_path = '../prev_input/test_workdir/data/in' #mounted so we can change after talking with albert about parcel compatibility
output_path = '../prev_output/test.json'
#CHANGE MODEL VAR IN CONFIG.PY to use a differen one
...
with open(output_path, 'w') as f:
    f.write("{would be patient info}"+f": {x[0][0]},")
    print(f"test succeeded: {x[0][0]}")
